...
var Mine = function  (ele,faceele,panewidth,paneheight,minenum,tagele,timeele) {
 ...
 Mine.prototype = {
    ...
    this.faceele.src = "img/game/mine/res/face_normal.bmp";
	...
	triggerClick:function(pos,e){
		...
		if(theCell.isMine == true){// 如果点到的这个是雷，就判输。显示所有雷
			this.faceele.src = "img/game/mine/res/face_fail.bmp";
			this.showMine();
		...
		if(openNum == okNum){
			this.faceele.src = "img/game/mine/res/face_success.bmp";
			alert("you win!");
			clearInterval(this.timer);
			this.ele.onmouseup = '';
			this.ele.onmousedown = '';
	...
	drawCell:function(pos,type){//绘制不同种类的格子。
		...
		var image = new Image();
		var src;
		var srcArr = ["img/game/mine/res/blank.bmp","img/game/mine/res/0.bmp","img/game/mine/res/flag.bmp","img/game/mine/res/ask.bmp","img/game/mine/res/mine.bmp","img/game/mine/res/blood.bmp","img/game/mine/res/error.bmp"];
	...
	drawNum:function(pos,num){//绘制数字
		...
		var cxt = this.cxt;
		var image = new Image();
		image.src = "img/game/mine/res/"+num+".bmp";
		image.onload = function(){
			cxt.drawImage(image,area[0],area[1],16,16);
		}
	},
	...
	numToImage:function (num,ele){
		...
		for (var i = 0,eLen=ele.length; i < eLen; i++) {
			ele[i].src="img/game/mine/res/d"+num.toString().charAt(i)+".bmp";
		};
